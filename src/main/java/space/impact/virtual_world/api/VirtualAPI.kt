...
import space.impact.virtual_world.api.FluidGenerator.createFluidRegion
import space.impact.virtual_world.api.FluidGenerator.getFluidVein
import space.impact.virtual_world.api.OreGenerator.SHIFT_REGION_FROM_CHUNK
import space.impact.virtual_world.api.OreGenerator.createOreRegion
import space.impact.virtual_world.api.OreGenerator.getVeinAndChunk
...
import space.impact.virtual_world.config.Config.IS_DISABLED_VIRTUAL_FLUIDS
import space.impact.virtual_world.config.Config.IS_DISABLED_VIRTUAL_ORES
import java.util.*
...
object VirtualAPI {
    ...
    fun extractFromChunk(chunk: Chunk, layer: Int, reduceCoefficient: Int = 1): Pair<VirtualOreVein?, Int> {
        return chunk.getVeinAndChunk(layer)?.let { (veinOre, chunkOre) ->
            getVirtualOreVeinById(veinOre.oreVein.id) to if (!chunkOre.hasExtract(reduceCoefficient)) 0 else chunkOre.size
        } ?: (null to 0)
    }
    ...
    fun extractFromVein(chunk: Chunk, layer: Int, reduceCoefficient: Int = 1): Pair<VirtualOreVein?, Int> {
        return chunk.getVeinAndChunk(layer)?.let { (veinOre, _) ->
            val vov = getVirtualOreVeinById(veinOre.oreVein.id)
            var reduce = 0
    ...
    fun getVirtualOreVeinInChunk(vein: VeinOre, layer: Int, dim: Int): VirtualOreVein? {
        return VIRTUAL_ORES.filter { it.layer == layer && it.dimensions.contains(dim) }.let { veins ->
            veins.find { it.id == vein.oreVein.id }
        }
    }
    ...
    @JvmStatic
    fun findRegionResourceFromChunk(regions: Set<ResourceRegion>, chunk: Chunk): ResourceRegion? {
        val x = chunk.xPosition shr SHIFT_REGION_FROM_CHUNK
        val y = chunk.zPosition shr SHIFT_REGION_FROM_CHUNK
        return regions.find { it.regionOre.xRegion == x && it.regionOre.zRegion == y }
    }
